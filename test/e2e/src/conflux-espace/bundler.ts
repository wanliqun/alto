...
export const startConfluxEspaceBundler = async ({
	...
	const child = execa(process.execPath, ["--import", "tsx/esm", altoEntry, "run"], {
		cwd: repoRoot,
		env: {
			...process.env,
	...
	const started = new Promise<void>((resolvePromise, rejectPromise) => {
		...
		const timeout = setTimeout(() => {
			rejectPromise(
				new Error(`Timed out waiting for bundler startup.\n\nRecent Alto output:\n${output.join("\n")}`),
			);
		}, 180_000);
		...
		const onExit = (code: number | null, signal: string | null) => {
			...
			rejectPromise(
				new Error(
					`Bundler exited before becoming ready${code !== null ? ` with code ${code}` : ""}${
